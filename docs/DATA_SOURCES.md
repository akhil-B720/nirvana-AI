...
| **Synthetic Development & Benchmark Architecture** | `dataset/synthetic/synthetic_*.csv` (`data_pipeline.synthetic`) | Deterministic Synthetic Generator | Python Pipeline / SQLite Ingest | `SYNTHETIC` | 5,000 projects across 37 States/UTs, 31,235 civil component states, 25,124 time-series snapshots, 10,000 documents, 8 controlled anomaly categories | NOT REAL RECORDS. Injected strictly for development, ML model training, and algorithmic benchmarking. | Strictly tagged `data_status = 'SYNTHETIC'` and `source_type = 'SYNTHETIC_TEST_DATA'`. Completely isolated from public verified government data. |
